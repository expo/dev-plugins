...
export function Logs({ active, logs, index, resetTo }: Props) {
  ...
  const hasSider = active && !!selectedItem;
              ...
              {({ index: itemIndex, style }) => {
                ...
                return (
                  <Row
                    ...
                    faded={index != null ? index > -1 && itemIndex > index : false}
                    onClick={() => {
...
const Row = styled.button<{ selected: boolean; faded: boolean }>((props) => ({
  ...
  backgroundColor: props.selected ? props.theme.token?.colorPrimary : 'inherit',
  ...
  '&:hover': {
    backgroundColor: props.selected ? props.theme.token?.colorPrimaryHover : 'rgba(0, 0, 0, 0.05)',
  },
}));

const JumpButton = styled.button((props) => ({
  appearance: 'none',
