...
import type { getActionFromState, getStateFromPath } from '@react-navigation/core';
...
export function LinkingTester({ linking, active }: Props) {
  ...
  const [state, setState] = React.useState<ReturnType<typeof getStateFromPath> | undefined>();

  const [action, setAction] = React.useState<ReturnType<typeof getActionFromState> | undefined>();
  ...
  return (
    <Layout hasSider={!!action}>
      ...
      {action ? <Sidebar action={action} state={state} /> : null}
    </Layout>
  );
}
...
const ErrorDescription = styled.p(({ theme: antdTheme }) => ({
